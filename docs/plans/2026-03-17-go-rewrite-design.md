...
**Goal:** Rewrite the current Ballerina-based GitHub-to-Discord webhook service into Go as the first workflow inside an extensible GitHub organization operations microservice, performing the migration in-place with batch commits and progressively removing the Ballerina implementation.
...
The service has no persistence layer, background jobs, or database. This makes it a strong candidate for a vertical-slice rewrite, and also means it can become the first workflow in a broader org-operations service without carrying a complicated migration burden.
...
The long-term product is not just a webhook notifier. It is a GitHub organization operations microservice that can eventually support workflows such as:

- pull request operations and merge-readiness workflows
- new member intake and onboarding
- organization notifications and ChatOps
- future integrations with the tools the team uses

For `v1`, only the current GitHub-to-Discord notification behavior will be implemented. The broader org-ops scope affects the architecture, not the initial feature set.
...
- `internal/service/`
  - orchestration of GitHub event handling and workflow dispatch
- `internal/workflows/`
  - workflow coordination for `v1` notifications and future org-ops flows
- `internal/integrations/`
  - tool-specific adapters used by workflows
- `internal/integrations/discord/`
  - Discord webhook client, message rendering, delivery
- `internal/domain/`
  - shared event models, workflow contracts, and constants where useful
- `internal/testdata/`
  - webhook fixtures for parity-focused tests

This is intentionally more layered than the current codebase while still keeping the service small. The key distinction is that workflows are separated from integrations so future org tooling does not need to be bolted onto a Discord-specific service core.
...
- Shape the service around reusable workflows and integration adapters rather than a one-off notifier

## Application Design

The `v1` Go request flow will be:
...
9. Route the event into a notification workflow
10. Render a Discord webhook payload through the Discord integration adapter
11. Deliver the notification to Discord
12. Return `200 OK` even when notification delivery fails, while logging the failure

This preserves the current external behavior while improving internal separation and leaves room for future workflows that are not notification-only.
...
- `internal/workflows/notify/` will decide whether an event should produce a notification
- `internal/integrations/discord/` will render embeds and send webhook requests
- `internal/service/` will coordinate the request path and invoke workflows
...
Internally, Go code will use explicit errors and narrow package responsibilities so failures are easier to test and trace. That same separation should make future PR and onboarding workflows cheaper to add.
...
- add workflow and integration boundaries for org-ops growth
...
4. Add workflow and integration boundaries
5. Add Discord event handling one slice at a time
6. Add tests and fixtures for migrated behavior
7. Switch container/runtime docs to Go
8. Remove Ballerina source and config files no longer needed
...
- Designing for future org-ops workflows adds abstraction pressure, so `v1` must stay intentionally limited to the current notifier scope
...
- the architecture cleanly separates workflows from integrations
